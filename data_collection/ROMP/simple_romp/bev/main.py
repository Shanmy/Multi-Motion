...
def bev_settings(input_args=sys.argv[1:]):
    ...
    parser.add_argument('--smpl_path', type=str, default='/home/notebook/data/group/pose_caption_dataset/ROMP_and_BEV/smpla_packed_info.pth', help = 'The path of SMPL-A model file')
    parser.add_argument('--smil_path', type=str, default='/home/notebook/data/group/pose_caption_dataset/ROMP_and_BEV/smil_packed_info.pth', help = 'The path of SMIL model file')
    parser.add_argument('--model_path', type=str, default='/home/notebook/data/group/pose_caption_dataset/ROMP_and_BEV/BEV.pth', help = 'The path of BEV checkpoint')
...
class BEV(nn.Module):
    def __init__(self, romp_settings, device):
    ...
    def _build_model_(self):
        ...
        model.load_state_dict(torch.load(self.settings.model_path, map_location=torch.device(self.tdevice)), strict=False)
    ...
    def _initilization_(self):        
        ...
        self.result_keys = ['smpl_thetas', 'smpl_betas', 'cam','cam_trans', 'params_pred', 'center_confs', 'pred_batch_ids', 'cam_czyx']
    ...
    def single_image_forward(self, image):
        parsed_results_all = self.model(image)

        for i, parsed_results in enumerate(parsed_results_all):
            ...
            parsed_results_all[i] = parsed_results
        return parsed_results_all
        
    @time_cost('BEV')
    @torch.no_grad()
    def forward(self, image, padding_all=None, signal_ID=0, **kwargs):
        outputs_all = self.process_normal_image(image, padding_all, signal_ID)
        ...
        for i, outputs in enumerate(outputs_all):
            ...
            if self.settings.render_mesh:
                ...
                # prep im for viz
                padding = [int(padding_all[i][j]) for j in range(len(padding_all[i]))]
                orig_sz_sq = max(padding[4], padding[5])
                image_viz = F.resize(image[i], (orig_sz_sq, orig_sz_sq))  # resize to orig size
                image_viz = image_viz[:, padding[0]:padding[1], padding[2]:padding[3]]
                image_viz = (127.5 * (image_viz.permute(1, 2, 0).cpu().numpy() + 1)).astype(np.uint8)  # convert to range [0, 255]
                image_viz = image_viz[:, :, [2, 1, 0]]  # switch from RGB to BGR
    
                outputs = rendering_romp_bev_results(self.renderer, outputs, image_viz, rendering_cfgs)

            outputs_all[i] = outputs
        return outputs_all
        
    def process_normal_image(self, image, padding=None, signal_ID=0):
        outputs_all = self.single_image_forward(image)
        if padding is not None:
            meta_data_all = [{'input2org_offsets': padding[i]} for i in range(image.shape[0])]
        else:
            # NOTE: make sure this is not needed for data collection w/o rendering
            assert not self.settings.render_mesh, 'must pass in padding argument to render mesh'
            meta_data_all = [{}] * images.shape[0]

        if self.settings.calc_smpl:
            for i, outputs in enumerate(outputs_all):
                if outputs is None:
                    continue
                meta_data = meta_data_all[i]
                ...
                outputs = self.filter_by_center_range(outputs)
                outputs_all[i] = outputs
        return outputs_all

    def filter_by_center_range(self, outputs):
        # only keep poses that are in the middle depth range (between low_z_thresh and high_z_thresh)
        cam_czyx = outputs['cam_czyx']
        kept_inds_x = ((cam_czyx[:, 2] >= self.low_x_thresh) * (cam_czyx[:, 2] <= self.high_x_thresh))
        kept_inds_y = ((cam_czyx[:, 1] >= self.low_y_thresh) * (cam_czyx[:, 1] <= self.high_y_thresh))
        kept_inds_z = ((cam_czyx[:, 0] >= self.low_z_thresh) * (cam_czyx[:, 0] <= self.high_z_thresh))
        kept_inds = (kept_inds_x * kept_inds_y * kept_inds_z).nonzero()[:, 0].cpu().numpy()

        if len(kept_inds) == 0:
            #print('No person in range detected!')
            return None

        for k in outputs.keys():
            if not k == 'smpl_face':
                outputs[k] = outputs[k][kept_inds]
