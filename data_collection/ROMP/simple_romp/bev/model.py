...
def get_cam3dmap_anchor(FOV, centermap_size):
    ...
    map_coord_range_each_level = (np.array([2/64., 25/64., 3/64., 2/64.], dtype=float) * centermap_size).astype(int)
...
class BEVv1(nn.Module):
    ...
    def forward_old(self, x):
        ...
        if hasattr(self, 'low_z_thresh'):
            kept_inds = ((cam_czyx[:, 0] >= self.low_z_thresh) * (cam_czyx[:, 0] <= self.high_z_thresh)).nonzero()[:,0].cpu().numpy()

            if len(kept_inds) == 0:
                #print('No person in middle depth detected!')
                return None

            params_preds = params_preds[kept_inds]
            cam_czyx = cam_czyx[kept_inds]
            pred_batch_ids = pred_batch_ids[kept_inds]
            pred_czyxs = pred_czyxs[kept_inds]
            center_confs = center_confs[kept_inds]
    ...
    @torch.no_grad()
    def forward(self, x):
        # first do all net ops
        x = self.backbone(x)
        center_maps_3d_all, cam_maps_3d_all, center_maps_fv_all = self.coarse2fine_localization(x)
        front_view_features_all = self.param_head(x)

        # then process net outputs, one image at a time
        output_list = []
        for i in range(x.shape[0]):
            center_maps_3d = center_maps_3d_all[i].unsqueeze(0)
            cam_maps_3d = cam_maps_3d_all[i].unsqueeze(0)
            center_maps_fv = center_maps_fv_all[i].unsqueeze(0)
            front_view_features = front_view_features_all[i].unsqueeze(0)
