...
class CenterMap3D(object):
    ...
    def parse_3dcentermap(self, center_maps):
        ...
        #topk_ys = (topk_inds.long()// w).float() # deprecated
        topk_ys = torch.div(topk_inds.long(), w, rounding_mode='trunc')
        ...
        topk_zs = torch.div(index.long(), K, rounding_mode='trunc')
        # topk_zs = index.long() // K # deprecated
...
def body_mesh_projection2image(j3d_preds, cam_preds, vertices=None, input2org_offsets=None, denormalize_trans=True):
    if denormalize_trans:
        pred_cam_t = denormalize_cam_params_to_trans(cam_preds, positive_constrain=False)
    else:
        pred_cam_t = cam_preds
