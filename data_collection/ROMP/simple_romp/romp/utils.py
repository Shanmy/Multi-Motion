...
class ResultSaver:
    ...
    def __call__(self, outputs, input_path, prefix=None, img_ext='.png',
        ...
        if caption is not None:
            # add border above and add caption
            rendered_image = cv2.copyMakeBorder(rendered_image, caption_pad, 0, 0, 0,
                                                cv2.BORDER_CONSTANT, value=[255, 255, 255])
            cv2.putText(img=rendered_image, text=caption, org=(caption_pad // 2, caption_pad // 2),
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.33, color=[0, 0, 0], thickness=1)
