...
test_folder = '/nfs/USRCSEA/IVA/Experiments/motion-diffusion/mitch-v3/multi-pose-v1/multi_pose_4_gpu/24-02-25-22-22-36/samples/multi_pose_8_gpu/24-03-04-22-32-31'
...
for folder in subfolders:
    if folder.startswith('cfg'):
        cfg_coefs.append(float(folder.split("_")[1]))
        cfg_stage_1_coefs.append(
            float(folder.split("_")[3]) if folder.split("_")[3] != 'None' else 0.0
        )
...
for metric in eval_dicts[0].keys():
    ...
    ax.scatter(cfg_coefs, cfg_stage_1_coefs, c=metric_vals, cmap='viridis_r')
    ax.set_xlabel('CFG Guidance Coef')
    ax.set_ylabel('CFG Stage 1 Guidance Coef')
