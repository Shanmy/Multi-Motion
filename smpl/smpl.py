# SMPL model, from https://github.com/Arthur151/ROMP/blob/master/simple_romp/romp/smpl.py
...
# SMPLA model, from: https://github.com/Arthur151/ROMP/blob/master/simple_romp/bev/post_parser.py

# keep SMPL files in the same folder as this code file by default
FPATH = os.path.dirname(os.path.realpath(__file__))
DEFAULT_SMPLA_PATH = os.path.join(FPATH, 'smpla_packed_info.pth')
DEFAULT_SMIL_PATH = os.path.join(FPATH, 'smil_packed_info.pth')

class SMPLA_parser(nn.Module):
    def __init__(self, smpla_path=None, smil_path=None):
        super(SMPLA_parser, self).__init__()
        if smpla_path is None:
            smpla_path = DEFAULT_SMPLA_PATH
        if smil_path is None:
            smil_path = DEFAULT_SMIL_PATH
    ...
    def forward(self, betas=None, thetas=None, root_align=True):
        ...
        if baby_mask.sum()>0:
            ...
            verts = torch.zeros(person_num, 6890, 3, device=thetas.device).float()
            joints = torch.zeros(person_num, 54+17, 3, device=thetas.device).float()
    ...
    # smpl forward pass plus global translation
    def forward_trans(self, betas, thetas, trans, center_trans=False):

        vertices, _, faces = self.forward(betas, thetas)

        if center_trans:
            trans = trans - torch.mean(trans, axis=0)  # centers translation over batch

        vertices = vertices + trans[:, np.newaxis, :]  # add translation to vertices
...
if __name__ == "__main__":
    ...
    # example of setting up smpl model
